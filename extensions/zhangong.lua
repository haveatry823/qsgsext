...
zgfunc[sgs.Damage].bgws=function(self, room, event, player, data,isOwner)
	local damage = data:toDamage()
	if damage.from and damage.from:objectName()==room:getOwner():objectName() and damage.from:isLord() and damage.to:getRole()=="loyalist" then
		addGameData("bgws",1)
	end		
end
...
zgfunc[sgs.DamageCaused].ljxs=function(self, room, event, player, data,isOwner)
	if not isOwner then return false end
	local damage = data:toDamage()
	if damage.card and damage.card:isKindOf("Slash") and damage.to:isKongcheng() and not damage.chain and not damage.transfer then
		local name="ljxs"
		addGameData(name,1)
		if getGameData(name)>=3 then			 
			addZhanGong(room,name)
...
zgfunc[sgs.Death].lczz=function(self, room, event, player, data,isOwner)
	...
	if killer:getRole()=="rebel" and (player:getRole()=="renegade" or player:getRole()=="loyalist") and killer:objectName()==room:getOwner():objectName()  then
		local name="lczz"
		addGameData(name,1)		
...
zgfunc[sgs.Death].cdzx=function(self, room, event, player, data,isOwner)
	...
	local killer=damage.from
	if not killer then return false end
	if killer:getRole()=="loyalist" and (player:getRole()=="renegade" or player:getRole()=="rebel") and killer:objectName()==room:getOwner():objectName()  then
		local name="cdzx"
		addGameData(name,1)		
		if getGameData(name,0)>=2  then 			 
			addZhanGong(room,name)
...
zgfunc[sgs.Death].xbtc=function(self, room, event, player, data,isOwner)
	...
	if getGameData("turncount")==1 and diffgroup and killerName==room:getOwner():objectName() and killerName==room:getCurrent():objectName() then
		local name="xbtc"
		addZhanGong(room,name)
	end		
end
...
zgfunc[sgs.FinishRetrial].tq=function(self, room, event, player, data,isOwner)
	...
	if judge.reason=="lightning" and room:getTag("retrial"):toBool()==false and judge.who:objectName()==room:getOwner():objectName() then
		setTurnData(name,1)
	else
		setTurnData(name,0)
	end
...
zgfunc[sgs.GameOverJudge].tongji=function(self, room, event, player, data,isOwner)
	...
	sqlexec("update results set general='%s',turncount=%d,alive=%d,result='%s',wen=wen+%d,wu=wu+%d,expval=expval+%d where id=%d",
			owner:getGeneralName(),getGameData("turncount"),alive,result,getTurnData("wen"),getTurnData("wu"),getTurnData("expval"),getGameData("roomid"))
...
zgfunc[sgs.TurnStart].init=function(self, room, event, player, data,isOwner)
	...
	sqlexec("update results set general='%s',turncount=%d,alive=%d,wen=wen+%d,wu=wu+%d,expval=expval+%d where id=%d",
			room:getOwner():getGeneralName(),getGameData("turncount"), alive,getTurnData("wen"),getTurnData("wu"),getTurnData("expval"),getGameData("roomid"))
...
zgfunc[sgs.SlashEffected].dqbr=function(self, room, event, player, data,isOwner)
	...
	local armor= (effect.to:getArmor() and effect.to:getArmor():isKindOf("RenwangShield")) or ((not effect.to:getArmor()) and effect.to:hasSkill("yizhong"))
...
function init_gamestart(self, room, event, player, data, isOwner)
	...
	if getGameData("roomid")==0 then 
		...
		sqlexec("insert into results values(%d,'%s','%s','%s',0,1,'-',0,0,0)",getGameData("roomid"),player:getGeneralName(),player:getRole(),room:getMode())
...
zgzhangong = sgs.CreateTriggerSkill{
	name = "#zgzhangong",
	events = {sgs.GameStart,sgs.TurnStart,sgs.CardFinished,sgs.Damage,sgs.GameOverJudge,sgs.Death,sgs.EventPhaseEnd,sgs.FinishRetrial,sgs.ChoiceMade,
			sgs.SlashEffected,sgs.DamageCaused},
	priority = 6,
